...
class Calculator {
  ...
  addToHistory(operand1, operation, operand2, result) {
    const historyItem = {
      expression: `${this.getDisplayNumber(operand1)} ${operation} ${this.getDisplayNumber(
        operand2
      )} =`,
  ...
  updateDisplay(showFullOperation = false) {
    ...
    if (showFullOperation && this.lastOperation != null) {
      ...
      this.previousOperandTextElement.innerText = `${this.getDisplayNumber(
        this.lastPreviousOperand
      )} ${this.lastOperation} ${this.getDisplayNumber(
        this.lastCurrentOperand
      )} =`;
