...
class Calculator {
    ...
    clear() {
        ...
        this.updateDisplay(); // Cập nhật hiển thị sau khi xóa
    }
    ...
    handleSingleOperandOperation(operation) {
        ...
        switch (operation) {
            case 'sqrt':  // hoặc '√'
                ...
                break;
            case '%':
                // Theo yêu cầu: "10 % → behaves as 10% of current operand", hiểu là chia cho 100
                result = current / 100;
                break;
            case 'x²':
...
// --- Kết nối Logic với các nút trên Giao diện ---
...
operationButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        // Phân loại phép toán: 2 ngôi (cần 2 số) và 1 ngôi (chỉ cần 1 số)
        const binaryOperations = ['+', '−', '×', '÷'];
        const unaryOperations = ['sqrt', '%', 'x²', '1/x'];
